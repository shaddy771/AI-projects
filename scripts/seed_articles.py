...
def build_faq(title: str, city: dict | None) -> list[dict]:
    ...
    if any(w in lower for w in ("стоит", "цена", "прайс", "сколько")):
        q1 = short if "?" in short else f"Сколько стоит {short.lower()}?"
    elif "?" in title:
        q1 = title.rstrip(".")
    else:
        q1 = f"Когда нужна помощь мастера: {short.lower()}?"
    return [
        {
            "q": q1,
...
LEGACY_FAQ = {
    "chto-delat-zaklinil-zamok": [
        {"q": "Можно ли смазать заклинивший замок WD-40?", "a": "Иногда помогает графитовая смазка. WD-40 зимой может замёрзнуть — лучше не использовать без просушки."},
        {"q": "Можно ли вскрыть заклинивший замок без повреждений?", "a": "Да, в большинстве случаев мастер открывает замок профессионально, не ломая дверь."},
        {"q": "Как быстро приедет мастер в Могилёве?", "a": "По Могилёву — от 15 до 20 минут. Работаем круглосуточно."},
    ],
    "kak-vybrat-zamok-dlya-dveri": [
        {"q": "Какой класс взломостойкости нужен для входной двери?", "a": "Рекомендуем замки не ниже 4 класса. Cisa, Kale K2, Гардиан 30.11 — проверенные варианты."},
        {"q": "Цилиндровый или сувальдный замок лучше?", "a": "Цилиндровый удобнее для замены личинки, сувальдный надёжнее. Часто ставят оба типа."},
        {"q": "Сколько стоит установка замка?", "a": "Профессиональный монтаж — от 45 BYN. Неправильная установка снижает класс защиты."},
    ],
    "slomalsya-klyuch-v-zamke": [
        {"q": "Можно ли извлечь обломок ключа самостоятельно?", "a": "Не рекомендуем — можно повредить личинку. Мастер извлекает обломок за 10–15 минут."},
        {"q": "Сколько стоит извлечение обломка ключа?", "a": "От 25 BYN. Точная цена зависит от типа замка и глубины застревания."},
        {"q": "Нужно ли менять замок после извлечения обломка?", "a": "Не всегда. Часто достаточно заменить личинку — от 25 BYN."},
    ],
    "skolko-stoit-vskrytie-zamka": [
        {"q": "Сколько стоит вскрытие замка в Могилёве?", "a": "От 30 BYN. Цилиндровый — от 35 BYN, сувальдный — от 45 BYN, авто — от 40 BYN."},
        {"q": "От чего зависит цена вскрытия?", "a": "От типа замка, двери, времени вызова (ночной тариф +15 BYN) и сложности работ."},
        {"q": "Мастер называет цену до начала работ?", "a": "Да, точную стоимость мастер озвучивает до вскрытия — без скрытых доплат."},
    ],
    "vskrytie-avto-bez-povrezhdeniy": [
        {"q": "Можно ли открыть машину без повреждений?", "a": "Да, профессионалы открывают 98% автомобилей без следов — специальными инструментами."},
        {"q": "Сколько стоит вскрытие автомобиля?", "a": "От 40 BYN. Зависит от марки, типа замка и времени вызова."},
        {"q": "Что делать, если ключи остались внутри?", "a": "Не бейте стекло — позвоните мастеру. Вскрытие в разы дешевле ремонта стекла."},
    ],
    "kak-zamenit-lichinku-zamka": [
        {"q": "Можно ли заменить личинку самостоятельно?", "a": "Да, если есть навыки и инструмент. Для бронированных дверей лучше вызвать мастера."},
        {"q": "Сколько стоит замена личинки?", "a": "Работа — от 25 BYN плюс стоимость новой личинки."},
        {"q": "Когда нужна замена личинки?", "a": "При потере ключей, переезде в новую квартиру или если ключи попали к посторонним."},
    ],
}
...
def migrate_legacy() -> list[dict]:
    ...
    for p in BLOG_POSTS:
        articles.append({
            ...
            "faq": LEGACY_FAQ.get(p["slug"], build_faq(p["title"], None)),
            "topic_type": "legacy",
        })
    return articles
